...
`)?.[0]?.trim()||"",{sellerName:c,sellerKey:m,isDirect:i}}return{sellerName:"",sellerKey:"",isDirect:!1}}function Y(e){let t=["near mint","lightly played","moderately played","heavily played","damaged","nm","lp","mp","hp"],r=e.toLowerCase();return t.some(n=>r.startsWith(n))}function Z(e){if(!e)return null;let t=e.getAttribute("href")||"",r=t.match(/\/product\/(\d+)/);if(r)return{productId:parseInt(r[1],10),customListingKey:null};let n=t.match(/\/product\/listing\/([^/]+)/);return n?{productId:null,customListingKey:n[1]}:null}function N(e){if(!e)return 0;let t=e.match(/\$?([\d,]+\.?\d*)/);return t&&parseFloat(t[1].replace(/,/g,""))||0}function V(){let e=document.querySelectorAll("p");for(let r of e){let n=r.textContent?.trim()||"";if(n.includes("Cart Subtotal")){let s=N(n);if(s>0)return s}}let t=document.querySelectorAll("h3");for(let r of t)if(r.textContent?.trim()==="Cart Summary"){let n=r.nextElementSibling;for(;n;){let s=n.textContent?.trim()||"";if(s.includes("Cart Subtotal")){let i=N(s);if(i>0)return i}n=n.nextElementSibling}}return 0}var k="https://mpgateway.tcgplayer.com";async function _(e){try{let t=W();if(!t)return{success:!1,error:"Could not find cart key. Please refresh the page and try again."};let r=[];for(let l of e.sellers)for(let d of l.items){if(!d.productConditionId){console.warn("[TCGmizer] Item missing productConditionId:",d);continue}r.push({sku:d.productConditionId,sellerId:l.sellerNumericId,sellerKey:l.sellerKey||l.sellerId,price:d.price,quantity:1,cardName:d.cardName,setName:d.setName||"",isDirect:d.directSeller||!1,customListingKey:d.customListingKey||null})}if(r.length===0)return{success:!1,error:"No items in optimized cart."};let n=new Map;for(let l of r){let d=l.customListingKey?`custom:${l.customListingKey}`:`${l.sku}:${l.sellerId}:${l.isDirect}`;if(n.has(d)){let g=n.get(d);g.quantity+=l.quantity,g.cardName+=`, ${l.cardName}`}else n.set(d,{...l})}let s=[...n.values()];console.log(`[TCGmizer] Applying cart: clearing then adding ${r.length} items (${s.length} unique sku+seller combos) via ${k}`);let i=await J(t);if(!i.success)return{success:!1,error:`Failed to clear cart: ${i.error}`};let m=new Set(s.map(l=>`${l.sku}:${l.sellerKey}`)),c=e.fallbackListings||{},o=0,a=0,u=[],h=[];for(let l=0;l<s.length;l++){let d=s[l],g=await A(t,d);if(g.success){l<s.length-1&&await T(50);continue}if(console.warn(`[TCGmizer] Failed to add item ${l+1}/${s.length}: ${g.error}`),console.warn(`[TCGmizer]   Item details: card="${d.cardName}", set="${d.setName}", sku=${d.sku}, sellerKey=${d.sellerKey}, sellerId=${d.sellerId}, price=${d.price}, qty=${d.quantity}, isDirect=${d.isDirect}`),console.warn(`[TCGmizer]   Error code: ${g.errorCode||"none"}`),g.errorCode==="CAPI-4"){let p=d.cardName.split(", ")[0],S=c[p]||[];console.log(`[TCGmizer]   Attempting CAPI-4 fallback for "${p}" \u2014 ${S.length} alternatives available`);let x=!1;for(let y of S){let E=`${y.sku}:${y.sellerKey}`;if(m.has(E))continue;console.log(`[TCGmizer]   Trying fallback: sku=${y.sku}, seller=${y.sellerKey} (${y.sellerName}), price=$${y.price}, set="${y.setName}"`);let U={sku:y.sku,sellerKey:y.sellerKey,price:y.price,quantity:d.quantity,cardName:d.cardName,setName:y.setName||d.setName,isDirect:y.isDirect||!1,customListingKey:y.customListingKey||null},L=await A(t,U);if(L.success){m.add(E),a++,h.push({cardName:p,originalSku:d.sku,originalSellerKey:d.sellerKey,originalPrice:d.price,fallbackSku:y.sku,fallbackSellerKey:y.sellerKey,fallbackPrice:y.price,fallbackSetName:y.setName,fallbackSellerName:y.sellerName}),console.log(`[TCGmizer]   \u2713 Fallback succeeded for "${p}" \u2014 new price: $${y.price} from ${y.sellerName}`),x=!0;break}m.add(E),console.warn(`[TCGmizer]   Fallback also failed (${L.errorCode||"unknown"}): sku=${y.sku}, seller=${y.sellerKey}`),await T(50)}if(x){l<s.length-1&&await T(50);continue}console.warn(`[TCGmizer]   All fallbacks exhausted for "${p}"`)}o++,u.push({cardName:d.cardName,setName:d.setName,sku:d.sku,sellerKey:d.sellerKey,price:d.price,errorCode:g.errorCode,reason:X(g.errorCode)||g.error}),l<s.length-1&&await T(50)}if(o===s.length)return{success:!1,error:`Failed to add all items. ${u[0]?.reason||"Unknown error"}`};if(a>0){console.log(`[TCGmizer] ${a} item(s) were replaced with fallback listings:`);for(let l of h)console.log(`[TCGmizer]   - ${l.cardName}: $${l.originalPrice} \u2192 $${l.fallbackPrice} (${l.fallbackSellerName}, ${l.fallbackSetName})`)}if(o>0){console.warn(`[TCGmizer] ${o}/${s.length} items failed to add:`);for(let l of u)console.warn(`[TCGmizer]   - ${l.cardName} (${l.setName}): ${l.errorCode||"unknown"} \u2014 ${l.reason}`);return{success:!0,partial:!0,failCount:o,totalCount:s.length,failedItems:u,fallbackCount:a,fallbackItems:h}}return a>0?{success:!0,partial:!0,failCount:0,totalCount:s.length,failedItems:[],fallbackCount:a,fallbackItems:h}:{success:!0,partial:!1,failCount:0,totalCount:s.length,failedItems:[],fallbackCount:0,fallbackItems:[]}}catch(t){return{success:!1,error:t.message}}}function P(e){try{sessionStorage.setItem("tcgmizer_undo_cart",JSON.stringify({timestamp:Date.now(),items:e}))}catch(t){console.warn("[TCGmizer] Failed to save cart state for undo:",t)}}function W(){let e=document.cookie.split(";");for(let t of e){let r=t.trim();if(r.startsWith("StoreCart_PRODUCTION=")){let n=r.substring(21),i=new URLSearchParams(n).get("CK");return i||n}}return null}async function J(e){try{let t=await fetch(`${k}/v1/cart/${e}/items/all`,{method:"DELETE",headers:{"Content-Type":"application/json",Accept:"application/json"},credentials:"include"});if(!t.ok){let r=await t.text().catch(()=>"");return{success:!1,error:`HTTP ${t.status}: ${r.substring(0,200)}`}}return{success:!0}}catch(t){return{success:!1,error:t.message}}}async function A(e,t){let r=!!t.customListingKey,n,s;r?(n=`${k}/v1/cart/${e}/listo/add`,s={customListingKey:t.customListingKey,priceAtAdd:t.price,quantityToBuy:t.quantity||1,channelId:0,countryCode:"US"}):(n=`${k}/v1/cart/${e}/item/add`,s={sku:t.sku,sellerKey:t.sellerKey,channelId:0,requestedQuantity:t.quantity||1,price:t.price,isDirect:t.isDirect||!1,countryCode:"US"}),console.log(`[TCGmizer] Adding to cart: "${t.cardName}" (${t.setName||"no set"}) \u2014 sku=${t.sku}, seller=${t.sellerKey}, price=$${t.price}, qty=${t.quantity||1}, isDirect=${t.isDirect||!1}${r?", customListingKey="+t.customListingKey:""}`),console.log(`[TCGmizer]   ${r?"Custom listing":"Standard"} \u2192 ${n.split("/").slice(-2).join("/")}`),console.log("[TCGmizer]   Request body:",JSON.stringify(s));try{let i=await fetch(n,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},credentials:"include",body:JSON.stringify(s)}),m=await i.text().catch(()=>"");if(console.log(`[TCGmizer]   Response ${i.status}: ${m.substring(0,500)}`),!i.ok){let c=null;try{c=JSON.parse(m)?.errors?.[0]?.code||null}catch{}return{success:!1,error:`HTTP ${i.status}: ${m.substring(0,200)}`,errorCode:c}}try{let c=JSON.parse(m);if(c?.errors&&c.errors.length>0){let o=c.errors[0]?.code||"";return{success:!1,error:`API error: ${c.errors[0]?.message||""} (${o})`,errorCode:o}}}catch{}return{success:!0}}catch(i){return console.error("[TCGmizer]   Network error:",i),{success:!1,error:i.message,errorCode:null}}}function X(e){switch(e){case"CAPI-4":return"Sold out (no longer available from this seller)";case"CAPI-17":return"Product not found (may have been delisted)";case"CAPI-35":return"Product not available for purchase";default:return`Error: ${e}`}}function T(e){return new Promise(t=>setTimeout(t,e))}var C="tcgmizer-panel";function G(){if(document.getElementById(C))return;let e=document.createElement("div");e.id=C,e.innerHTML=`
  ...
  `,document.body.appendChild(e);let t=e.querySelector(".tcgmizer-header"),r=!1,n=0,s=0;t.addEventListener("mousedown",i=>{i.target.closest(".tcgmizer-close")||(r=!0,n=i.clientX-e.offsetLeft,s=i.clientY-e.offsetTop,t.style.cursor="grabbing",i.preventDefault())}),document.addEventListener("mousemove",i=>{r&&(e.style.left=i.clientX-n+"px",e.style.top=i.clientY-s+"px",e.style.right="auto")}),document.addEventListener("mouseup",()=>{r&&(r=!1,t.style.cursor="")}),e.querySelector(".tcgmizer-close").addEventListener("click",()=>{e.style.display="none"}),e.querySelector(".tcgmizer-start").addEventListener("click",()=>{typeof e._onStart=="function"&&e._onStart()}),e.querySelector(".tcgmizer-retry").addEventListener("click",()=>{e._hasConfig?(f(e,".tcgmizer-error"),v(e,".tcgmizer-config")):typeof e._onStart=="function"&&e._onStart()})}function q(e){let t=document.getElementById(C);t&&(t._onStart=e)}function R(){let e=document.getElementById(C);e&&(e.style.display="flex")}function I(e,t,r){let n=document.getElementById(C);if(!n)return;f(n,".tcgmizer-idle"),f(n,".tcgmizer-config"),f(n,".tcgmizer-results"),f(n,".tcgmizer-error"),v(n,".tcgmizer-progress"),n.querySelector(".tcgmizer-progress-text").textContent=e||"Working...";let s=n.querySelector(".tcgmizer-progress-bar");t!=null&&r!=null&&r>0?(s.style.width=`${Math.round(t/r*100)}%`,s.classList.remove("tcgmizer-progress-bar-indeterminate")):(s.style.width="100%",s.classList.add("tcgmizer-progress-bar-indeterminate"))}function F(e,t){let r=document.getElementById(C);if(!r)return;r._hasConfig=!0,f(r,".tcgmizer-idle"),f(r,".tcgmizer-progress"),f(r,".tcgmizer-results"),f(r,".tcgmizer-error"),v(r,".tcgmizer-config");let n=r.querySelector(".tcgmizer-config"),s=e.languages.map(c=>{let o=c==="English"?"checked":"";return`<label class="tcgmizer-checkbox-label">
  ...
  `,chrome.storage.local.get("optimizerSettings",c=>{let o=c.optimizerSettings;o&&(o.languages&&o.languages.length>0&&n.querySelectorAll('.tcgmizer-lang-options input[type="checkbox"]').forEach(a=>{a.checked=o.languages.includes(a.value)}),o.conditions&&o.conditions.length>0&&n.querySelectorAll('.tcgmizer-cond-options input[type="checkbox"]').forEach(a=>{a.checked=o.conditions.includes(a.value)}),o.minimizeVendors!=null&&(n.querySelector(".tcgmizer-minimize-vendors").checked=o.minimizeVendors,n.querySelector(".tcgmizer-max-cuts").disabled=!o.minimizeVendors),o.maxCuts!=null&&(n.querySelector(".tcgmizer-max-cuts").value=String(o.maxCuts)),o.exactPrintings!=null&&(n.querySelector(".tcgmizer-exact-printings").checked=o.exactPrintings))});function m(c){let o=n.querySelector(".tcgmizer-exclude-banned"),a=n.querySelector(".tcgmizer-ban-count-label");!o||!a||(c.length===0?(o.checked=!1,o.disabled=!0,a.textContent="(none banned)"):(o.disabled=!1,o.checked=!0,a.textContent=`(${c.length} banned)`),o._bannedKeys=c.map(u=>u.sellerKey))}chrome.storage.sync.get("bannedSellers",c=>{m(c.bannedSellers||[])}),chrome.storage.onChanged.addListener((c,o)=>{o==="sync"&&c.bannedSellers&&m(c.bannedSellers.newValue||[])}),n.querySelector(".tcgmizer-manage-ban-link").addEventListener("click",c=>{c.preventDefault(),chrome.runtime.sendMessage({type:"OPEN_OPTIONS_PAGE"})}),n.querySelectorAll(".tcgmizer-select-all").forEach(c=>{c.addEventListener("click",o=>{o.preventDefault();let a=c.dataset.target;n.querySelectorAll(`.tcgmizer-${a}-options input[type="checkbox"]`).forEach(u=>u.checked=!0)})}),n.querySelectorAll(".tcgmizer-select-none").forEach(c=>{c.addEventListener("click",o=>{o.preventDefault();let a=c.dataset.target;n.querySelectorAll(`.tcgmizer-${a}-options input[type="checkbox"]`).forEach(u=>u.checked=!1)})}),n.querySelector(".tcgmizer-minimize-vendors").addEventListener("change",c=>{n.querySelector(".tcgmizer-max-cuts").disabled=!c.target.checked}),n.querySelector(".tcgmizer-run-solver").addEventListener("click",()=>{let c=[...n.querySelectorAll(".tcgmizer-lang-options input:checked")].map(p=>p.value),o=[...n.querySelectorAll(".tcgmizer-cond-options input:checked")].map(p=>p.value),a=n.querySelector(".tcgmizer-minimize-vendors").checked,u=parseInt(n.querySelector(".tcgmizer-max-cuts").value,10)||0,h=n.querySelector(".tcgmizer-exact-printings").checked,l=n.querySelector(".tcgmizer-exclude-banned"),d=l.checked&&l._bannedKeys?l._bannedKeys:[];if(c.length===0){alert("Please select at least one language.");return}if(o.length===0){alert("Please select at least one condition.");return}chrome.storage.local.set({optimizerSettings:{languages:c,conditions:o,minimizeVendors:a,maxCuts:u,exactPrintings:h}});let g={languages:c.length===e.languages.length?[]:c,conditions:o.length===e.conditions.length?[]:o,minimizeVendors:a,maxCuts:a?u:0,exactPrintings:h,bannedSellerKeys:d};typeof t=="function"&&t(g)}),n.querySelector(".tcgmizer-refetch").addEventListener("click",()=>{typeof r._onStart=="function"&&r._onStart()})}function M(e,t){let r=document.getElementById(C);if(!r)return;f(r,".tcgmizer-idle"),f(r,".tcgmizer-progress"),f(r,".tcgmizer-config"),f(r,".tcgmizer-error"),v(r,".tcgmizer-results");let n=r.querySelector(".tcgmizer-results");if(!e.success){n.innerHTML=`
  ...
  `,n.querySelector(".tcgmizer-apply").addEventListener("click",()=>{confirm("This will replace your current TCGPlayer cart with the optimized selections. This cannot be undone! Continue?")&&typeof t=="function"&&t(e)}),n.querySelector(".tcgmizer-back-to-config").addEventListener("click",()=>{f(r,".tcgmizer-results"),v(r,".tcgmizer-config")})}function D(e,t){let r=document.getElementById(C);if(!r)return;f(r,".tcgmizer-idle"),f(r,".tcgmizer-progress"),f(r,".tcgmizer-config"),f(r,".tcgmizer-error"),v(r,".tcgmizer-results");let n=r.querySelector(".tcgmizer-results");if(!e||e.length===0){n.innerHTML=`
      <div class="tcgmizer-result-error">
        <p>No feasible solutions found.</p>
      </div>
    `;return}let s=e[e.length-1],i="";for(let a=0;a<e.length;a++){let u=e[a],h=u.totalCost-s.totalCost,l=h>.005?`+$${h.toFixed(2)}`:"Cheapest",d=h>.005?"":"tcgmizer-cheapest-tag",g=u.cutCards&&u.cutCards.length>0?`<div class="tcgmizer-cut-info" title="${z(u.cutCards.join(" \xB7 "))}">\u2702\uFE0F Cut ${u.cutCards.length} card${u.cutCards.length!==1?"s":""}: ${z(u.cutCards.join(" \xB7 "))}</div>`:"",p="";for(let S of u.sellers)p+=K(S,!1);i+=`
      <div class="tcgmizer-compare-row" data-index="${a}">
        <div class="tcgmizer-compare-row-summary">
          <span class="tcgmizer-compare-vendors">${u.sellerCount} vendor${u.sellerCount!==1?"s":""}</span>
          <span class="tcgmizer-compare-price">$${u.totalCost.toFixed(2)}</span>
        ...
        <div class="tcgmizer-compare-detail" style="display:none">
          <div class="tcgmizer-summary-row tcgmizer-summary-detail" style="margin-bottom:8px">
            Items: $${u.totalItemCost.toFixed(2)} \xB7 Shipping: $${u.totalShipping.toFixed(2)}
          </div>
          <div class="tcgmizer-sellers-list">${p}</div>
        </div>
      </div>
  ...
  `,n.querySelectorAll(".tcgmizer-compare-row").forEach(a=>{let u=a.querySelector(".tcgmizer-compare-row-summary"),h=a.querySelector(".tcgmizer-compare-detail"),l=a.querySelector(".tcgmizer-compare-toggle");u.addEventListener("click",d=>{if(d.target.closest(".tcgmizer-compare-apply"))return;let g=h.style.display!=="none";h.style.display=g?"none":"block",l.textContent=g?"\u25B6":"\u25BC",a.classList.toggle("tcgmizer-compare-row-expanded",!g)})}),n.querySelectorAll(".tcgmizer-compare-apply").forEach(a=>{a.addEventListener("click",u=>{let h=parseInt(u.target.closest(".tcgmizer-compare-row").dataset.index,10),l=e[h];confirm(`Apply cart with ${l.sellerCount} vendor${l.sellerCount!==1?"s":""} ($${l.totalCost.toFixed(2)})? This will replace your current TCGPlayer cart. This cannot be undone!`)&&typeof t=="function"&&t(l)})}),n.querySelector(".tcgmizer-back-to-config").addEventListener("click",()=>{f(r,".tcgmizer-results"),v(r,".tcgmizer-config")})}function b(e){let t=document.getElementById(C);t&&(f(t,".tcgmizer-idle"),f(t,".tcgmizer-progress"),f(t,".tcgmizer-config"),f(t,".tcgmizer-results"),v(t,".tcgmizer-error"),t.querySelector(".tcgmizer-error-text").textContent=e)}function v(e,t){let r=e.querySelector(t);r&&(r.style.display="block")}function f(e,t){let r=e.querySelector(t);r&&(r.style.display="none")}function z(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}var Q={"Near Mint":"NM","Lightly Played":"LP","Moderately Played":"MP","Heavily Played":"HP",Damaged:"DMG",Mint:"M","Near Mint Foil":"NM-F","Lightly Played Foil":"LP-F","Moderately Played Foil":"MP-F","Heavily Played Foil":"HP-F","Damaged Foil":"DMG-F"};function ee(e){return e?Q[e]||e:""}function K(e,t){let r=te(e.items),n=re(r,t),s=e.freeShipping?'<span class="tcgmizer-free-shipping">FREE shipping</span>':`Shipping: $${e.shippingCost.toFixed(2)}`,i=e.isDirect?" tcgmizer-seller-direct":"",m=e.isDirect?`<img src="https://mp-assets.tcgplayer.com/img/direct-icon-new.svg" alt="Direct" style="height:14px;vertical-align:middle;margin-right:4px" />${z(e.sellerName)}`:z(e.sellerName);return`
  ...
  `}function te(e){let t=[],r=new Map;for(let n of e){let s=`${n.productId}|${n.condition}|${n.language}|${n.price}|${n.productConditionId}`;r.has(s)?t[r.get(s)].qty+=1:(r.set(s,t.length),t.push({item:n,qty:1}))}return t}function re(e,t){return e.map(({item:r,qty:n})=>{let s=t&&r.printingChanged?` <span class="tcgmizer-changed" title="Different printing (originally ${z(O(r.originalSetName)||"unknown set")})">\u{1F500}</span>`:"",i=n>1?`<span class="tcgmizer-item-qty">${n}\xD7</span> `:"",m=[ee(r.condition),O(r.setName),r.language].filter(Boolean).join(" \xB7 "),c=`https://tcgplayer-cdn.tcgplayer.com/product/${r.productId}_200w.jpg`,o=n>1?`$${r.price.toFixed(2)} ea`:`$${r.price.toFixed(2)}`;return`
    ...
    `,a.addEventListener("mouseenter",()=>{a.style.background="#258a50"}),a.addEventListener("mouseleave",()=>{a.style.background="#2e9e5e"}),a.addEventListener("click",()=>{R(),t()}),c.appendChild(a),m.insertAdjacentElement("afterend",c),!0}if(!i()){let m=new MutationObserver(()=>{i()&&m.disconnect()});m.observe(document.body,{childList:!0,subtree:!0})}}function t(){I("Reading cart...",null,null);let s;try{s=w()}catch(i){b(`Error reading cart: ${i.message}`),console.error("[TCGmizer] Cart read error:",i);return}if(!s.cartItems||s.cartItems.length===0){let i=document.querySelector("main"),m=i?i.querySelectorAll("article").length:0,c=document.querySelectorAll('a[href*="/product/"]').length,o=document.querySelectorAll("li").length;b(`Could not read cart items. Debug: main=${!!i}, articles=${m}, productLinks=${c}, li=${o}. Make sure you have items in your cart.`);return}P(s.cartItems),console.log(`[TCGmizer] Read ${s.cartItems.length} items from cart, total: $${s.currentCartTotal}`),chrome.runtime.sendMessage({type:$.START_OPTIMIZATION,cartData:s},i=>{if(chrome.runtime.lastError){b(`Failed to start: ${chrome.runtime.lastError.message}`);return}i?.error&&b(i.error)})}function r(s){I("Optimizing...",null,null),chrome.runtime.sendMessage({type:$.SOLVE_WITH_CONFIG,config:s},i=>{if(chrome.runtime.lastError){b(`Failed to start solver: ${chrome.runtime.lastError.message}`);return}i?.error&&b(i.error)})}chrome.runtime.onMessage.addListener((s,i,m)=>{switch(s.type){case"PING":return m({ok:!0}),!1;case $.TOGGLE_PANEL:{let c=document.getElementById("tcgmizer-panel");c&&(c.style.display==="none"||c.style.display===""?(c.style.display="flex",t()):c.style.display="none"),m({ok:!0});break}case $.OPTIMIZATION_PROGRESS:I(s.message||`${s.stage}...`,s.current,s.total);break;case $.LISTINGS_READY:F(s.options,r);break;case $.OPTIMIZATION_RESULT:M(s.result,n);break;case $.OPTIMIZATION_MULTI_RESULT:D(s.results,n);break;case $.OPTIMIZATION_ERROR:b(s.error||"An unknown error occurred.");break}return!1});async function n(s){I("Applying optimized cart...",null,null);let i=await _(s);if(!i.success){b(`Failed to apply cart: ${i.error}`);return}if(i.partial){let m=i.totalCount-i.failCount,c=i.fallbackCount||0,o="";if(c>0){o+=`${c} item(s) were sold out and replaced with the next-cheapest listing:
...
`;for(let a of i.failedItems){let u=a.setName?` (${a.setName})`:"";o+=`  \u2022 ${a.cardName}${u}: ${a.reason}
`}o+=`
