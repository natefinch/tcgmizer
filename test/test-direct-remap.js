...
console.log('Test 1: No Direct listings');
{
  const sellers = {
    ...
    s2: { sellerName: 'Store B', shippingCost: 2.00, freeShippingThreshold: 10.00 },
  };

  const listings = [
    { listingId: 'l1', sellerId: 's1', price: 1.50, directListing: false },
    { listingId: 'l2', sellerId: 's2', price: 2.00, directListing: false },
    { listingId: 'l3', sellerId: 's1', price: 3.00 }, // no directListing field at all
  ];
...
{
  const sellers = {
    s1: { sellerName: 'Store A', sellerKey: 's1', sellerNumericId: 101, shippingCost: 1.50, freeShippingThreshold: null },
    s2: { sellerName: 'Store B', sellerKey: 's2', sellerNumericId: 102, shippingCost: 2.00, freeShippingThreshold: null },
  };

  const listings = [
    { listingId: 'l1', sellerId: 's1', sellerKey: 's1', sellerNumericId: 101, sellerName: 'Store A', price: 1.50, directListing: true },
    { listingId: 'l2', sellerId: 's2', sellerKey: 's2', sellerNumericId: 102, sellerName: 'Store B', price: 2.00, directListing: true },
    { listingId: 'l3', sellerId: 's1', sellerKey: 's1', sellerNumericId: 101, sellerName: 'Store A', price: 3.00, directListing: true },
  ];
  ...
  test('All listings remapped to synthetic Direct seller', () => {
    for (const l of result.listings) {
      assert(l.sellerId === '__tcgplayer_direct__', `Listing ${l.listingId} sellerId should be __tcgplayer_direct__, got ${l.sellerId}`);
    }
  });
  ...
  test('Original seller info preserved on remapped listings', () => {
    ...
    assert(l1.originalSellerNumericId === 101, `Expected originalSellerNumericId=101, got ${l1.originalSellerNumericId}`);
  ...
  test('Synthetic Direct seller added to sellers map', () => {
    ...
    assert(direct.freeShippingThreshold === 50.00, `Expected freeShippingThreshold=50, got ${direct.freeShippingThreshold}`);
  });
  ...
  test('Other listing properties preserved', () => {
    const l1 = result.listings.find(l => l.listingId === 'l1');
    assert(l1.price === 1.50, `Expected price=1.50, got ${l1.price}`);
...
{
  const sellers = {
    s1: { sellerName: 'Mixed Store', sellerKey: 's1', sellerNumericId: 201, shippingCost: 1.00, freeShippingThreshold: 5.00 },
  };

  const listings = [
    { listingId: 'l1', sellerId: 's1', sellerKey: 's1', sellerNumericId: 201, sellerName: 'Mixed Store', price: 2.00, directListing: true },
    { listingId: 'l2', sellerId: 's1', sellerKey: 's1', sellerNumericId: 201, sellerName: 'Mixed Store', price: 3.00, directListing: false },
    { listingId: 'l3', sellerId: 's1', sellerKey: 's1', sellerNumericId: 201, sellerName: 'Mixed Store', price: 1.00, directListing: true },
  ];
...
{
  const sellers = {
    sA: { sellerName: 'Pure Direct Store', sellerKey: 'sA', sellerNumericId: 301, shippingCost: 1.50, freeShippingThreshold: null },
    sB: { sellerName: 'Mixed Store', sellerKey: 'sB', sellerNumericId: 302, shippingCost: 2.00, freeShippingThreshold: 8.00 },
    sC: { sellerName: 'Regular Store', sellerKey: 'sC', sellerNumericId: 303, shippingCost: 1.00, freeShippingThreshold: null },
  };

  const listings = [
    // sA: only Direct listings → should be remapped
    { listingId: 'l1', sellerId: 'sA', sellerKey: 'sA', sellerNumericId: 301, sellerName: 'Pure Direct Store', price: 1.00, directListing: true },
    { listingId: 'l2', sellerId: 'sA', sellerKey: 'sA', sellerNumericId: 301, sellerName: 'Pure Direct Store', price: 2.00, directListing: true },
    // sB: mixed → Direct items ALSO remapped (Direct always ships from TCGPlayer warehouse)
    { listingId: 'l3', sellerId: 'sB', sellerKey: 'sB', sellerNumericId: 302, sellerName: 'Mixed Store', price: 3.00, directListing: true },
    { listingId: 'l4', sellerId: 'sB', sellerKey: 'sB', sellerNumericId: 302, sellerName: 'Mixed Store', price: 4.00, directListing: false },
    // sC: only non-Direct → untouched
    { listingId: 'l5', sellerId: 'sC', sellerKey: 'sC', sellerNumericId: 303, sellerName: 'Regular Store', price: 5.00, directListing: false },
  ];
  ...
  test('Synthetic Direct seller created with correct shipping', () => {
    ...
    assert(direct.freeShippingThreshold === 50.00, `Expected 50.00, got ${direct.freeShippingThreshold}`);
  });
...
{
  const sellers = {
    s1: { sellerName: 'Direct Seller 1', sellerKey: 's1', sellerNumericId: 401, shippingCost: 1.00, freeShippingThreshold: null },
    s2: { sellerName: 'Direct Seller 2', sellerKey: 's2', sellerNumericId: 402, shippingCost: 2.00, freeShippingThreshold: null },
    s3: { sellerName: 'Direct Seller 3', sellerKey: 's3', sellerNumericId: 403, shippingCost: 1.50, freeShippingThreshold: null },
  };

  const listings = [
    { listingId: 'l1', sellerId: 's1', sellerKey: 's1', sellerNumericId: 401, sellerName: 'Direct Seller 1', price: 1.00, directListing: true },
    { listingId: 'l2', sellerId: 's2', sellerKey: 's2', sellerNumericId: 402, sellerName: 'Direct Seller 2', price: 2.00, directListing: true },
    { listingId: 'l3', sellerId: 's3', sellerKey: 's3', sellerNumericId: 403, sellerName: 'Direct Seller 3', price: 3.00, directListing: true },
  ];
...
{
  const sellers = {
    s1: { sellerName: 'Mixed A', shippingCost: 1.00, freeShippingThreshold: 5.00 },
    s2: { sellerName: 'Mixed B', shippingCost: 1.50, freeShippingThreshold: 10.00 },
  };

  const listings = [
    { listingId: 'l1', sellerId: 's1', sellerKey: 's1', sellerNumericId: 601, sellerName: 'Mixed A', price: 1.00, directListing: true },
    { listingId: 'l2', sellerId: 's1', price: 2.00, directListing: false },
    { listingId: 'l3', sellerId: 's2', sellerKey: 's2', sellerNumericId: 602, sellerName: 'Mixed B', price: 3.00, directListing: true },
    { listingId: 'l4', sellerId: 's2', price: 4.00, directListing: false },
  ];
...
{
  const sellers = { s1: { sellerName: 'Store', shippingCost: 1.00, freeShippingThreshold: null } };
  const listings = [];
...
{
  const sellers = {
    s1: { sellerName: 'No-Key Store', shippingCost: 1.00, freeShippingThreshold: null },
  };

  const listings = [
    { listingId: 'l1', sellerId: 's1', sellerNumericId: 501, sellerName: 'No-Key Store', price: 1.00, directListing: true },
    // no sellerKey field
  ];
...
console.log('\nTest 9: ILP end-to-end — mixed seller Direct items go to Direct seller');
{
  ...
  const sellers = {
    mixed: { sellerName: 'Mixed Store', sellerKey: 'mixed', sellerNumericId: 901, shippingCost: 2.00, freeShippingThreshold: 5.00 },
    regular: { sellerName: 'Regular Store', sellerKey: 'regular', sellerNumericId: 902, shippingCost: 1.00, freeShippingThreshold: null },
  };

  const listings = [
    // Card A: Direct listing from mixed seller (will be remapped to Direct)
    { listingId: 'l1', sellerId: 'mixed', sellerKey: 'mixed', sellerNumericId: 901, sellerName: 'Mixed Store', slotId: 'c1', price: 3.00, skuId: 'sku1', condition: 'NM', setName: 'SET', language: 'EN', directListing: true, productId: 9001, productConditionId: 90010 },
    // Card A: non-Direct listing from regular store
    { listingId: 'l1b', sellerId: 'regular', sellerKey: 'regular', sellerNumericId: 902, sellerName: 'Regular Store', slotId: 'c1', price: 2.50, skuId: 'sku1b', condition: 'NM', setName: 'SET', language: 'EN', directListing: false, productId: 9001, productConditionId: 90010 },
    // Card B: non-Direct listing from mixed seller
    { listingId: 'l2', sellerId: 'mixed', sellerKey: 'mixed', sellerNumericId: 901, sellerName: 'Mixed Store', slotId: 'c2', price: 3.00, skuId: 'sku2', condition: 'NM', setName: 'SET', language: 'EN', directListing: false, productId: 9002, productConditionId: 90020 },
  ];
  ...
  const result = parseSolution(solution, variableMap, cardSlots, remapped.sellers, 20.00);
  ...
  test('Solution is optimal', () => {
    ...
    for (const s of result.sellers) {
      console.log(`    ${s.sellerName}: $${s.subtotal.toFixed(2)} + $${s.shippingCost.toFixed(2)} = $${s.sellerTotal.toFixed(2)}${s.isDirect ? ' [DIRECT]' : ''}`);
    }
  });
}
...
console.log('\nTest 10: ILP end-to-end — pure-Direct sellers merge into one');
{
  ...
  const sellers = {
    dA: { sellerName: 'Direct A', sellerKey: 'dA', sellerNumericId: 801, shippingCost: 5.00, freeShippingThreshold: null },
    dB: { sellerName: 'Direct B', sellerKey: 'dB', sellerNumericId: 802, shippingCost: 4.00, freeShippingThreshold: null },
  };

  const listings = [
    { listingId: 'l1', sellerId: 'dA', sellerKey: 'dA', sellerNumericId: 801, sellerName: 'Direct A', slotId: 'c1', price: 2.00, skuId: 'sku1', condition: 'NM', setName: 'SET', language: 'EN', directListing: true, productId: 8001, productConditionId: 80010 },
    { listingId: 'l2', sellerId: 'dB', sellerKey: 'dB', sellerNumericId: 802, sellerName: 'Direct B', slotId: 'c2', price: 3.00, skuId: 'sku2', condition: 'NM', setName: 'SET', language: 'EN', directListing: true, productId: 8002, productConditionId: 80020 },
  ];
